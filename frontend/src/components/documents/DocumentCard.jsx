...
const formatFileSize = (bytes) => {
    ...
    return `${size.toFixed(1)} ${units[unitIndex]}`; //toFixed(1) -> decimal place 1
}
    ...
    onClick={handleNavigate}>
      ...
      <div className="">
        <div className="flex flex-start justify-between gap-3 mb-4">
          ...
          <button className='opacity group-hover:opacity-100 w-8 h-8 flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all duration-200' onClick={handleDelete}>
        ...
        <div className="flex items-center gap-3 text-xs text-slate-500 mb-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 px-2. py-1.5 bg-purple-50 rounded-lg">
              <BrainCircuit className='w-3.5 h-3.5 text-purple-600' strokeWidth={2}></BrainCircuit>
              <span className=' text-xs text-semibold fond-semibold text-purple-700'>{document.flashcardCount} Flashcards</span>
            </div>
          )}
            { document.quizCount!== undefined && (
              <div className="flex items-center gap-1.5 px-2. py-1.5 bg-purple-50 rounded-lg">
                <BrainCircuit className='w-3.5 h-3.5 text-purple-600' strokeWidth={2}></BrainCircuit>
                <span className='text-xs text-semibold fond-semibold text-purple-700'>{document.quizCount} Quizzes</span>
              </div>
          )}
        </div>
      ...
      {/* Footer */}
      <div className="mt-5 pt-5 border-slate-100">
        <div className="flex items-center gap-1.5 text-xs text-slate-500">
          <Clock className='w-3.5 h-3.5' strokeWidth={2}></Clock>
